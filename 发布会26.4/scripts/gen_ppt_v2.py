...
subtitle2_box = slide.shapes.add_textbox(Inches(1), Inches(4.3), Inches(8), Inches(0.6))
...
print("Slide 2: Philosophy")

# ========== Slide 3: Architecture ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "总体架构：四层设计")
...
# ========== Slide 4: Vertical LLM ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "垂直大模型：水利领域专业化")

# Left: General LLM
left_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), Inches(1.2), Inches(4.5), Inches(2.8))
left_box.fill.solid()
left_box.fill.fore_color.rgb = C['light_bg']
left_box.line.color.rgb = C['text']
...
# Right: Vertical LLM
right_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(5.5), Inches(1.2), Inches(4), Inches(2.8))
...
items = ["水利专业知识", "规范标准内化", "工程经验积累", "规则引擎兜底"]
for item in items:
    p = tf.add_paragraph()
    p.text = f"✓ {item}"
    p.font.size = Pt(14)
...
# Bottom: Features
feature_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), Inches(4.5), Inches(9), Inches(2.5))
feature_box.fill.solid()
feature_box.fill.fore_color.rgb = C['orange']
feature_box.line.color.rgb = C['orange']
feature_box.line.width = Pt(2)

tf = feature_box.text_frame
tf.text = "HydroClaw垂直大模型特点"
p = tf.paragraphs[0]
p.font.size = Pt(18)
p.font.bold = True
p.font.color.rgb = C['white']
...
features = [
    "1. 水利规范知识库：设计规范、运行规程、安全标准",
    "2. 工程案例库：南水北调、胶东调水等实际工程经验",
    "3. 物理约束：水力学、水文学基本定律硬约束",
...
print("Slide 4: Vertical LLM")

# ========== Slide 5: Personal Agent ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "个人水网智能体：每个人的AI助手")

# Top: Concept
concept_box = slide.shapes.add_textbox(Inches(0.8), Inches(1.2), Inches(8.4), Inches(0.8))
tf = concept_box.text_frame
tf.text = "不同角色选择不同Skill，自动获得对应能力和约束"
p = tf.paragraphs[0]
p.font.size = Pt(18)
p.font.bold = True
...
# Four roles
roles = [
    ("设计工程师", ["泵站选型", "断面优化", "管网计算"], "个人超级工具", C['blue']),
    ("运维人员", ["ODD预警", "故障诊断", "调度优化"], "智能运维助手", C['orange']),
    ("科研人员", ["仿真对比", "参数辨识", "方案评估"], "科研实验平台", C['green']),
    ("学生教师", ["教学实验", "概念演示", "作业批改"], "智能教学助手", C['purple'])
]

for i, (role, skills, value, color) in enumerate(roles):
    row = i // 2
    col = i % 2
    x = 0.8 + col * 4.6
    y = 2.4 + row * 2.2

    box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(4.2), Inches(1.8))
    box.fill.solid()
    box.fill.fore_color.rgb = color
    box.line.color.rgb = color
    box.line.width = Pt(2)
...
print("Slide 5: Personal Agent")

# ========== Slide 6: Cognitive Layer ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "第①层：认知决策层")
...
process_box.fill.fore_color.rgb = C['light_bg']
...
print("Slide 6: Cognitive Layer")

# ========== Slide 7: Skills Layer ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "第②层：技能编排层")
...
print("Slide 7: Skills Layer")

# ========== Slide 8: Engines Layer ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "第③层：七大通用引擎")
...
tf.text = "[OK] 每个引擎内部可切换后端（开源/商业）  [OK] 通过MCP协议集成外部工具"
p = tf.paragraphs[0]
p.font.size = Pt(14)
p.font.color.rgb = C['text']
p.alignment = PP_ALIGN.CENTER

print("Slide 8: Engines Layer")

# ========== Slide 9: Components Layer ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "第④层：三族元件库")
...
print("Slide 9: Components Layer")

# ========== Slide 10: Runtime Example ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "运行示例：断面优化Skill")
...
print("Slide 10: Runtime Example")

# ========== Slide 11: Design Institute Strategy ==========
slide = prs.slides.add_slide(blank)
add_header(slide, "设计院切入策略：三阶段渗透")
...
print("Slide 11: Design Institute Strategy")
...
print("Slide 12: Summary")

# Save
output = "HydroClaw_认知智能方案_完整版.pptx"
prs.save(output)
print(f"\nPPT saved: {output}")
